# test/test_classic_tls.py
"""
Benchmark pro KLASICKOU kryptografii (tls/tls_classic_c.py).
Využívá knihovnu 'cryptography' pro srovnání s PQC.
"""
import time
import tracemalloc
import os
...
# Import klasické implementace
from tls import tls_classic_c as tls_module
from .test_utils import _stats, _print_table

# Nastavení benchmarku
ROUNDS = 100  # C knihovny jsou rychlé, můžu testovat více kol
VARIANTS = [0, 1, 2]
VARIANT_LABELS = [
    "ECDSA-P256 + X25519",
    "ECDSA-P384 + X25519",
...
def main():
    print(f"=== TEST: KLASICKÁ KRYPTOGRAFIE ({ROUNDS} kol) ===")
    results_time = []
    results_mem = []
    table_rows = []
    ...
    if any(results_time):
        ...
        plt.bar(VARIANT_LABELS, results_time, color="#3CB371")
        plt.title("Klasika (C) - Průměrný čas handshake")
        plt.ylabel("Čas [ms]")
        plt.tight_layout()
        plt.savefig("graphs/classical_c_time.png")
        print("\nGraf času uložen: graphs/classical_c_time.png")

    # Graf paměti
    if any(results_mem):
        plt.figure(figsize=(8, 5))
        plt.bar(VARIANT_LABELS, results_mem, color="#3CB371")
        plt.title("Klasika (C) - Průměrná špičková paměť")
        plt.ylabel("Paměť [KiB]")
        plt.tight_layout()
        plt.savefig("graphs/classical_c_mem.png")
        print("Graf paměti uložen: graphs/classical_c_mem.png")

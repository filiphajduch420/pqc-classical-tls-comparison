...
ROUNDS = 1000
VARIANTS = [
    {"id": 0, "name": "ECDSA-P256 + X25519"},
    {"id": 1, "name": "ECDSA-P384 + X25519"},
    {"id": 2, "name": "Ed25519 + X25519"}
]
...
def plot_results(results):
    ...
    bars = plt.bar(variant_names, times, color='#3CB371')
    plt.title('Klasika (C) - Celkový čas handshake')
    plt.ylabel('Čas [ms]')
    plt.grid(axis='y', linestyle='--', alpha=0.5)
    plt.bar_label(bars, padding=3, fmt='%.3f', fontsize=9)
    plt.tight_layout()
    plt.savefig("graphs/tls_classic_time.png", dpi=150)
    plt.close()
    print("Graf času uložen: graphs/tls_classic_time.png")

    # Graf paměti
    mems = [results[v["name"]]["memory"] for v in VARIANTS]

    plt.figure(figsize=(10, 6))
    bars = plt.bar(variant_names, mems, color='#3CB371')
    plt.title('Klasika (C) - Špičková paměť handshake')
    plt.ylabel('Paměť [KiB]')
    plt.grid(axis='y', linestyle='--', alpha=0.5)
    plt.bar_label(bars, padding=3, fmt='%.2f', fontsize=9)
    plt.tight_layout()
    plt.savefig("graphs/tls_classic_memory.png", dpi=150)
    plt.close()
    print("Graf paměti uložen: graphs/tls_classic_memory.png")


def main():
    print(f"=== BENCHMARK TLS: Klasická kryptografie ({ROUNDS} kol) ===")
    all_results = {}

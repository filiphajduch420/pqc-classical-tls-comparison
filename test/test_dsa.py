import time
import tracemalloc
import os
import matplotlib.pyplot as plt
import numpy as np
import oqs
from typing import List, Tuple

# Import mé implementace
from tls import mldsa
from tls.mldsa_files.constants import get_params_by_id

# Nastavení počtu opakování
ROUNDS_PYTHON = 5  # DSA v Pythonu je výpočetně velmi náročné
ROUNDS_OQS = 1000  # C je rychlé

# Definice variant k testování
VARIANTS = [
    {"id": 0, "name": "ML-DSA-44", "oqs_name": "ML-DSA-44"},
    {"id": 1, "name": "ML-DSA-65", "oqs_name": "ML-DSA-65"},
    {"id": 2, "name": "ML-DSA-87", "oqs_name": "ML-DSA-87"},
]

# Dummy data pro podpis
MESSAGE = b"Testovaci zprava pro benchmark podpisu."
CONTEXT = b"BenchCtx"  # Moje implementace vyžaduje kontext


def measure_python_dsa(variant_id, rounds):
    """Změří průměrné časy pro mou Python implementaci."""
    times = {"KeyGen": [], "Sign": [], "Verify": []}
    params = get_params_by_id(variant_id)

    print(f"  -> Měřím Python (ID {variant_id}, {rounds} kol)...")

    for _ in range(rounds):
        # 1. KeyGen
        t0 = time.perf_counter()
        pk, sk = mldsa.ML_DSA_KeyGen(params)
        times["KeyGen"].append((time.perf_counter() - t0) * 1000)

        # 2. Sign
        t0 = time.perf_counter()
        sig = mldsa.ML_DSA_Sign(sk, MESSAGE, CONTEXT, params)
        times["Sign"].append((time.perf_counter() - t0) * 1000)

        # 3. Verify
        t0 = time.perf_counter()
        valid = mldsa.ML_DSA_Verify(pk, MESSAGE, sig, CONTEXT, params)
        times["Verify"].append((time.perf_counter() - t0) * 1000)

        if not valid:
            print("! CHYBA: Neplatný podpis v Python benchmarku")

    return {k: sum(v) / len(v) for k, v in times.items()}


def measure_oqs_dsa(oqs_name, rounds):
    """Změří průměrné časy pro OQS (C) implementaci."""
    times = {"KeyGen": [], "Sign": [], "Verify": []}

    print(f"  -> Měřím OQS ({oqs_name}, {rounds} kol)...")

    signer = oqs.Signature(oqs_name)

    for _ in range(rounds):
        # 1. KeyGen
        t0 = time.perf_counter()
        pk = signer.generate_keypair()
        times["KeyGen"].append((time.perf_counter() - t0) * 1000)

        # 2. Sign (OQS wrapper obvykle nebere kontext explicitně pro standardní varianty)
        t0 = time.perf_counter()
        sig = signer.sign(MESSAGE)
        times["Sign"].append((time.perf_counter() - t0) * 1000)

        # 3. Verify
        t0 = time.perf_counter()
        valid = signer.verify(MESSAGE, sig, pk)
        times["Verify"].append((time.perf_counter() - t0) * 1000)

    return {k: sum(v) / len(v) for k, v in times.items()}


def plot_comparison(results):
    """Vykreslí a uloží SAMOSTATNÝ graf pro každou operaci."""
    ops = ["KeyGen", "Sign", "Verify"]
    variant_names = [v["name"] for v in VARIANTS]

    os.makedirs("graphs", exist_ok=True)

    # Projdeme každou operaci a uděláme jí vlastní graf
    for op in ops:
        py_vals = []
        oqs_vals = []

        for v in VARIANTS:
            name = v["name"]
            py_vals.append(results[name]["Python"][op])
            oqs_vals.append(results[name]["OQS"][op])

        plt.figure(figsize=(10, 6))

        x = np.arange(len(variant_names))
        width = 0.35

        bars1 = plt.bar(x - width / 2, py_vals, width, label='Python (Moje)', color='#4B9CD3')
        bars2 = plt.bar(x + width / 2, oqs_vals, width, label='OQS (C)', color='#FFB347')

        plt.title(f'ML-DSA: {op} - Porovnání času (Logaritmická osa)')
        plt.ylabel('Čas [ms]')
        plt.xticks(x, variant_names)
        plt.yscale('log')  # Logaritmická osa
        plt.legend()
        plt.grid(axis='y', linestyle='--', alpha=0.5)

        # Popisky hodnot
        plt.bar_label(bars1, padding=3, fmt='%.2f', fontsize=9)
        plt.bar_label(bars2, padding=3, fmt='%.3f', fontsize=9)

        plt.tight_layout()

        # Uložení
        filename = f"graphs/dsa_{op.lower()}.png"
        plt.savefig(filename, dpi=150)
        plt.close()
        print(f"Graf uložen: {filename}")


def main():
    print("=== BENCHMARK ML-DSA: Python vs OQS ===")
    all_results = {}

    for var in VARIANTS:
        v_name = var["name"]
        print(f"\nTestuji variantu: {v_name}")

        res_py = measure_python_dsa(var["id"], ROUNDS_PYTHON)
        res_oqs = measure_oqs_dsa(var["oqs_name"], ROUNDS_OQS)

        all_results[v_name] = {
            "Python": res_py,
            "OQS": res_oqs
        }

        # Textový výpis
        print(f"{'Operace':<10} | {'Python [ms]':<15} | {'OQS [ms]':<15} | {'Zrychlení (x)':<10}")
        print("-" * 60)
        for op in ["KeyGen", "Sign", "Verify"]:
            t_py = res_py[op]
            t_oqs = res_oqs[op]
            speedup = t_py / t_oqs if t_oqs > 0 else 0
            print(f"{op:<10} | {t_py:<15.3f} | {t_oqs:<15.4f} | {speedup:<10.1f}")

    plot_comparison(all_results)


if __name__ == "__main__":
    main()